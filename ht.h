...
// To be completed
template <typename KeyType>
struct QuadraticProber : public Prober<KeyType> {
    HASH_INDEX_T next() 
    {
        ...
        if(this->numProbes_ >= this->m_/2) {
            return this->npos; 
        }
        HASH_INDEX_T loc = (this->start_ + this->numProbes_*this->numProbes_) % this->m_;
        this->numProbes_++;
        return loc;
    }
};
...
public:
    ...
    void init(HASH_INDEX_T start, HASH_INDEX_T m, const KeyType& key) 
    {
        ...
        // To be completed - Compute probe stepsize given modulus and h2(k) 
        dhstep_ = modulus - h2_(key) % modulus;
        //std::cout << "dhstep for " << key << " is " << dhstep_ << std::endl;
    }
    ...
    HASH_INDEX_T next() 
    {
        ...
        // compute the next probe location
...
private:
    ...
    double rAlpha_;  // max loading factor before resize
    size_t size_; // number of non-deleted key,value pairs (i.e. n)
    size_t deleted_; // number of deleted + non-deleted key,value pairs (i.e. n)
};
...
{
    if((double)(size_+deleted_) / CAPACITIES[mIndex_] >= rAlpha_ ) {
        resize();
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    if(nullptr == item){
        // do nothing if item doesn't exist
        return;
    }
    item->deleted = true;
    size_--;
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    // intialize the new table
    std::vector<HashItem*> newTable(CAPACITIES[mIndex_+1]);
    for(HASH_INDEX_T i=0; i < CAPACITIES[mIndex_+1]; i++){
        newTable[i] = nullptr;
    }

    // would normally have swapped at the end, but probe always looks
    // at table_ but I really want it to look at newTable_, so swap now
    table_.swap(newTable);
    // TESTING: Easy to screw up and not use the wrong Capacity when rehashing 
    mIndex_++;

    // rehash the old table (due to swap, newTable is really the oldTable)
    std::vector<HashItem*>& oldTable = newTable;
    size_t newSize = 0;
    for(HASH_INDEX_T i=0; i < CAPACITIES[mIndex_-1]; i++){
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    //std::cout << "Starting probe on key " << key << " hashes to " << h << std::endl;
    ...
    {
        //std::cout << "\tTrying location " << loc << " of " << CAPACITIES[mIndex_] << std::endl;
